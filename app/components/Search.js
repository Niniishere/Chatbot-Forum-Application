...
function Search() {
  ...
  useEffect(() => {
    ...
    //when the user closes the search with the click and then click the esc key (we want to remove the eventlistener)
      ...
      <div className="search-overlay-bottom">
        <div className="container container--narrow py-3">
          ...
          <div className={"live-search-results " + (state.show == "results" ? "live-search-results--visible" : "")}>
            ...
            {!Boolean(state.results.length) && <p className="alert alert-danger text-center shadow-sm"> Sorry, we could not find any result. </p>}
          </div>
        </div>
      </div>
    </div>
